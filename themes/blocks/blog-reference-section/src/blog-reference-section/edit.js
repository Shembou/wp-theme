...
export default function Edit({ attributes, setAttributes }) {
	...
	const available_categories = useSelect(select =>
	...
	const post_links = posts?.map(({ link, content }, index) => {
		...
		<>
			<InspectorControls>
				<PanelBody>
					<PanelRow>
						<TextControl
							label={__('Tag', 'blog-reference-section')}
							value={tag || ''}
					...
					<PanelRow>
						<TextControl
							label={__('Heading', 'blog-reference-section')}
							value={heading || ''}
					...
					<PanelRow>
						<TextControl
							label={__('Paragraph', 'blog-reference-section')}
							value={paragraph || ''}
					...
					<PanelRow>
						<TextControl
							label={__('Posts Per Page Limit', 'blog-reference-section')}
							value={posts_per_page_limit || ''}
							type='number'
							onChange={(value) => setAttributes({ posts_per_page_limit: value })}
						/>
					</PanelRow>
				</PanelBody>
			</InspectorControls>
			<section {...useBlockProps()} id="blog-reference-section" >
				<header>
					...
					<div className='categories'>
						...
						<div className="categories-wrapper">
							{available_categories && available_categories.map(({ name, slug }, index) => (
								<p key={index} className='tag'>{name}</p>
				...
				<div className='blog-posts-wrapper'>
					{post_links && post_links.slice(0, parseInt(posts_per_page_limit)).map(({ link, data }, index) => (
			...
			<p className='important'>Pagination controls only visible on frontend</p>
		</>
	);
}
